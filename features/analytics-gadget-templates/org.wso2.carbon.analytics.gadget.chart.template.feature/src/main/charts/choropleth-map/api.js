...
(function() {
    ...
    getConfig = function(schema) {
        var chartConf = require(CHART_LOCATION + '/choropleth-map/config.json').config;
    ...
    buildChartConfig = function (_chartConfig) {
        ...
        conf.charts[0] = {
            type : "map",
            y: _chartConfig.y,
            mapType: _chartConfig.mapType
        };

         var jsonLocation = document.location.protocol+"//"+document.location.host 
                            + '/portal/extensions/chart-templates/choropleth-map/libs/';

        if (_chartConfig.mapType = "world") {
            conf.geoCodesUrl = jsonLocation + 'world.json';
            conf.helperUrl = jsonLocation + 'countryInfo.json';
            conf.mapType = "world";
        } else  if (_chartConfig.mapType = "usa") {
            conf.geoCodesUrl = jsonLocation + 'usa.json';
            conf.helperUrl = jsonLocation + 'usaInfo.json';
            conf.mapType = "usa";
        } else if (_chartConfig.mapType = "europe") {
            conf.geoCodesUrl = jsonLocation + 'europe.json';
            conf.helperUrl = jsonLocation + 'countryInfo.json';
            conf.mapType = "europe";
        }
