...
public class SparkScriptCAppDeployer implements AppDeploymentHandler {
        ...
        for (Artifact.Dependency dep : artifacts) {
            ...
            if (TYPE.equals(artifact.getType())) {
                deployer = AppDeployerUtils.getArtifactDeployer(axisConfiguration,
                        AnalyticsConstants.SCRIPT_DEPLOYMENT_DIR,
            ...
            // call the script deployer to deploy
            if (deployer != null) {
    ...
    private boolean isAccepted(String serviceType) {
        if (acceptanceList == null) {
            acceptanceList = AppDeployerUtils.buildAcceptanceList(ServiceHolder.getRequiredFeatures());
        }
