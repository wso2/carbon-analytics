...
public class MySQLMyISAMAnalyticsDataSourceTest extends AnalyticsDataSourceTest {
    ...
    private QueryConfiguration generateQueryConfiguration() {
        ...
        initQueries[3] = "CREATE TABLE AN_FS_PATH (path VARCHAR(256), is_directory BOOLEAN, length BIGINT, parent_path VARCHAR(256), PRIMARY KEY(path), FOREIGN KEY (parent_path) REFERENCES AN_FS_PATH(path) ON DELETE CASCADE) ENGINE='MyISAM'";
        initQueries[4] = "CREATE TABLE AN_FS_DATA (path VARCHAR(256), sequence BIGINT, data VARBINARY(1024), PRIMARY KEY (path,sequence), FOREIGN KEY (path) REFERENCES AN_FS_PATH(path) ON DELETE CASCADE) ENGINE='MyISAM'";
