...
//    }
    ...
    private QueryConfiguration generateQueryConfiguration() {
        ...
        initQueries[0] = "CREATE TABLE AN_TABLE_RECORD (record_id VARCHAR(50), table_category VARCHAR(256), table_name VARCHAR(256), timestamp BIGINT, data BLOB, PRIMARY KEY(record_id, table_category, table_name)) ENGINE='InnoDB'";
        ...
        initQueries[4] = "CREATE TABLE AN_FS_PATH (path VARCHAR(256), is_directory BOOLEAN, length BIGINT, parent_path VARCHAR(256), PRIMARY KEY(path), FOREIGN KEY (parent_path) REFERENCES AN_FS_PATH(path) ON DELETE CASCADE) ENGINE='InnoDB'";
        initQueries[5] = "CREATE TABLE AN_FS_DATA (path VARCHAR(256), sequence BIGINT, data BLOB, PRIMARY KEY (path,sequence), FOREIGN KEY (path) REFERENCES AN_FS_PATH(path) ON DELETE CASCADE) ENGINE='InnoDB'";
