...
public class DefaultUIOutputAuthorizationServiceImpl implements UIOutputAuthorizationService {
    ...
    @Override
    public boolean authorizeSubscription(JSONArray eventJson, Map<Integer, String> filterProps,
                                         String username, int tenantId) throws UIAdaptorException {
        Set<Integer> filterIndexes = filterProps.keySet();
        for (Integer aFilterIndex : filterIndexes) {
            try {
                if (!eventJson.getString(aFilterIndex).equalsIgnoreCase(filterProps.get(aFilterIndex))) {
                    return false;
                }
            } catch (JSONException e) {
                throw new UIAdaptorException("Unable validate the stream filter properties for event : " + eventJson
                        + " ", e);
            }
        }
        return true;
