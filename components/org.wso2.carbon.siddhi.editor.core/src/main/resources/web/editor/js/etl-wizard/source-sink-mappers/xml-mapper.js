...
define(['require', 'jquery', 'lodash', 'log', 'alerts'],
    ...
    function (require, $, _, log, Alerts) {
        ...
        XMLMapper.prototype.render = function () {
            ...
            if (config.customEnabled) {
                switch (this.__mapperType) {
                    case 'source':
                        this.renderSourceMapper();
                        break;
                    case 'sink':
                        this.renderSinkMapper();
                        break;
                }
            }
        }

        XMLMapper.prototype.renderSinkMapper = function () {

        }

        XMLMapper.prototype.renderSourceMapper = function () {
            console.log('Rendering custom mapping section for - XML');
            var self = this;
            var container = this.__mapperContainer;
            ...
            container.find('#source-mapper-attribute-dropdown>.dropdown-item')
                .on('click', function (evt) {
                    ...
                    console.log(config.mapping.attributes);
                    self.render();
                });
        ...
        XMLMapper.prototype.generateHTMLFromXML = function(xmlObject, indent, id) {
            ...
            for (var i = 0; i < xmlObject.childElementCount; i++) {
                ...
                if(xmlObject.children[i].childElementCount > 0) {
                    ...
                    content += `<span id="custom-map-val-${key}" class="custom-mapper-val ${config.mapping.attributes[attrib_key] ? 'ok-to-clear': 'no-clear'}" id="attrib-val-${i}">${config.mapping.attributes[attrib_key] ? `{{  ${config.mapping.attributes[attrib_key].attributeName}  }}` : '{{value}}' }</span>`;
                }
