...
define(['require', 'lodash', 'appData', 'log', 'constants', 'handlebar', 'annotationObject', 'annotationElement',
    ...
    function (require, _, AppData, log, Constants, Handlebars, AnnotationObject, AnnotationElement, DesignViewUtils,
        ...
        FormUtils.prototype.renderSourceSinkStoreTypeDropDownForWizard = function (id, predefinedTypes, formContainer) {
            var selectionFormTemplate = Handlebars.compile($('#type-selection-form-template').html())
            ({id: id, types: predefinedTypes});
            formContainer.find('#define-' + id).html(selectionFormTemplate);
        };
        ...
        FormUtils.prototype.renderAnnotationTemplateForWizard = function (className, annotations, formContainer) {
            var self = this;
            var raw_partial = document.getElementById('recursiveAnnotationPartial').innerHTML;
            Handlebars.registerPartial('recursiveAnnotation', raw_partial);
            var annotationFormTemplate = Handlebars.compile($('#annotation-form-template').html())(annotations);
            formContainer.find('.' + className).html(annotationFormTemplate);
            self.loadAnnotationForWizard(formContainer);
        };
        ...
        FormUtils.prototype.renderOptionsForWizard = function (optionsArray, customizedOptions, id ,formContainer) {
            var self = this;
            ...
            formContainer.find('#' + id + '-options-div').html(optionsTemplate);
        ...
        FormUtils.prototype.selectTypesOfSavedAttributes = function (savedAttributes) {
            ...
            form.find('.attribute .attr-content').each(function () {
        ...
        FormUtils.prototype.selectTypesOfSavedAttributesForWizard = function (savedAttributes,formContainer) {
            //to select the options(type) of the saved attributes
            var i = 0;
        ...
        FormUtils.prototype.validateOptions = function (predefinedOptions, id) {
            ...
            var isError = false;
            var form = $('#form-action');
            form.find('#' + id + '-options .option').each(function () {
        ...
        FormUtils.prototype.validateAttributes = function (attributeNameList) {
            ...
            var isErrorOccurred = false;
            var form = $('#form-action');
            form.find('.attr-name').each(function () {
        ...
        FormUtils.prototype.buildMapSectionForWizard = function (predefinedMaps,formContainer, mapperOptions) {
            var self = this;
            var customizedMapperOptions = [];
            self.renderMapForWizard(predefinedMaps, formContainer);
            mapperOptions = self.getSelectedTypeParameters(Constants.DEFAULT_MAPPER_TYPE, predefinedMaps);
            var mapperOptionsWithValues = self.createObjectWithValues(mapperOptions);
            self.renderOptionsForWizard(mapperOptionsWithValues, customizedMapperOptions, Constants.MAPPER, formContainer)
        };
        ...
        FormUtils.prototype.loadAnnotationForWizard = function (formContainer) {
            var self = this;
            //initialise jstree
            formContainer.find("#annotation-div").jstree({
                "core": {
                    "check_callback": true
                },
                "themes": {
            ...
            var tree = formContainer.find('#annotation-div').jstree(true);
            self.addEventListenersForJstreeForWizard(tree, formContainer);
        };
        ...
        FormUtils.prototype.hideCustomizedOptionsDivForWizard = function (formContainer) {
            formContainer.find('.customized-options').remove();
        };
