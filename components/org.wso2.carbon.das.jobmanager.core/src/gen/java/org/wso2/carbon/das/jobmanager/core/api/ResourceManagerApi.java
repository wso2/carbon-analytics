...
public class ResourceManagerApi implements Microservice {
    ...
    @io.swagger.annotations.ApiOperation(value = "Get the satus.", notes = "Returns status of manager nodes cluster and the resourcepool.", response = Deployment.class, tags = {"ResourceManager",})
    @io.swagger.annotations.ApiResponses(value = {
            @io.swagger.annotations.ApiResponse(code = 200, message = "Successful operation", response = Deployment.class)})
    ...
    @Produces({"application/json"})
    @io.swagger.annotations.ApiOperation(value = "Register resource node and Updates Heartbeat.", notes = "Resource nodes will call this endpoint to get them self registerd in the resource pool. Once registered, consecetive calls for this endpoints will be used as heartbeat from tose resource nodes.", response = HeartbeatResponse.class, tags = {"ResourceManager",})
    @io.swagger.annotations.ApiResponses(value = {
            @io.swagger.annotations.ApiResponse(code = 200, message = "Resource successfully added / Heartbeat updated.", response = HeartbeatResponse.class),

            @io.swagger.annotations.ApiResponse(code = 301, message = "Not the current leader (redir. to correct leader).", response = HeartbeatResponse.class),

            @io.swagger.annotations.ApiResponse(code = 400, message = "Leader not found", response = HeartbeatResponse.class)})
    public Response updateHeartbeat(@ApiParam(value = "Node object that needs to be registered or update heartbeats as the resource.", required = true) NodeConfig node
    )
            throws NotFoundException {
